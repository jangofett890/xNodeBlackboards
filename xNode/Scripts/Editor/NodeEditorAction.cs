...
namespace XNodeEditor {
    public partial class NodeEditorWindow {
        ...
        public void Controls() {
            ...
            //Added to allow overridden/custom controls (Subgraph in Blackboard)
            for (int ne = 0; ne < graph.nodes.Count; ne++) {
                // Skip null nodes. The user could be in the process of renaming scripts, so removing them at this point is not advisable.
                if (graph.nodes[ne] == null) continue;
                if (ne >= graph.nodes.Count) return;
                XNode.Node node = graph.nodes[ne];
                NodeEditor nodeEditor = NodeEditor.GetEditor(node, this);
                nodeEditor.Controls(e);
            }
